function h=som_show_chl(sMap, varargin)
...
%                       colorbar. By default denormalized values ahfig2 = figure;
...
for i=1:n,                         % main loop
  ...
  case 'ng'
    tmp_h=som_cplane(sMap.topol.lattice,sMap.topol.msize, sMap.valclust);
    set(tmp_h,'EdgeColor', General.edgecolor);
    set(h_axes(i),'Tag','Cplane');
    h_label(i,1)=xlabel('Clusters');
  
  case 'ngc'
    unitcon = sum(sMap.ngconnect,2);  
    tmp_h=som_cplane(sMap.topol.lattice,sMap.topol.msize, unitcon);
    set(tmp_h,'EdgeColor', General.edgecolor);
    set(h_axes(i),'Tag','Cplane');
    h_label(i,1)=xlabel('Connectivité');

  case 'ngd'
    U = som_umat(sMap);
    Um = U(1:2:size(U,1),1:2:size(U,2));
    Um = 1./Um;
    tmp_h=som_cplane(sMap, Um(:));
    set(tmp_h,'EdgeColor', General.edgecolor);
    set(h_axes(i),'Tag','Cplane');
    h_label(i,1)=xlabel('1/Distance');

  case 'ngp'
    U = som_umat(sMap);
    Um = U(1:2:size(U,1),1:2:size(U,2));
    unitden = sum(sMap.ngconnect,2)./(Um(:).^2);
    tmp_h=som_cplane(sMap, unitden);
    set(tmp_h,'EdgeColor', General.edgecolor);
    set(h_axes(i),'Tag','Cplane');
    h_label(i,1)=xlabel('Density');
    
  case 'ngpe'
    %svc = sign(sMap.valclust+1);
    svc=1;
    tmp_h=som_cplane(sMap,sMap.density.*svc+svc-1);
    %tmp_h=som_cplane(sMap,sMap.densitysave);
    set(tmp_h,'EdgeColor', General.edgecolor);
    set(h_axes(i),'Tag','Cplane');
    h_label(i,1)=xlabel('Density accentuated');
  ...
  case 'ngpei'
    ...
    %svc = sign(sMap.valclust+1); 
    svc=1;
    tmp_h=som_grid(lattice, msize, 'surf', sMap.density.*svc+svc-1, ...
	'Marker', 'none', 'Line', 'none');
    set(h_axes(i),'Tag','CplaneI');
    h_label(i,1)=xlabel('Density interpolated');
...
SOM_SHOW.comp_norm= General.comp(find(General.comp>0));%sMap.comp_norm; %(General.comp(find(General.comp>0)));
...
for i=1:2:length(args),
  ...
  switch identifier  
   case {'comp','compi','ng','ngc','ngp','ngd','ngpe','ngpei'}
