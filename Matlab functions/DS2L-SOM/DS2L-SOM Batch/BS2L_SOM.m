function sM = BS2L_SOM(Data, sM, varargin)
...
if ismember('norm',varargin), Data = som_normalize(Data,'var'); end;
...
while i0+1<=dlen, 
  ...
  Dist = (M.^2)*W1(:,inds) - M*WD(:,inds) + W2*D2(:,inds); % Distance euclidienne au carrée
  Dist = Dist*(2/ddim); % Dist(dim)² = dim/2 * dist(2)², on pondére les distances par le rapport entre la dim des données et la dim de la carte
...
sM.valclust = S2L_Clust(sM, sM.ngconnect); 

%%%%%%%%%%%% À NE PAS UTILISER POUR UTILISER LES CONNEXIONS %%%%%%%%%%%%%%%
...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%% Post traitement de la densit� %%%%%%%%%%%%%%%%%%%%%%%%%%%%%% %%
denclust = sM.density;

%Etape 1, calcul des zones emmer�es            
