...
for i=1:n,                         % main loop
  ...
  case 'ngp'
    ...
    set(tmp_h,'EdgeColor', General.edgecolor);
    set(h_axes(i),'Tag','Cplane');
    h_label(i,1)=xlabel('Densité');
    
  case 'ngpe'
    svc = sign(sMap.valclust+1);
    tmp_h=som_cplane(sMap,sMap.density.*svc+svc);
    set(tmp_h,'EdgeColor', General.edgecolor);
    set(h_axes(i),'Tag','Cplane');
    h_label(i,1)=xlabel('Densité étendue');
  
  case 'ngpei'
      
    %%% Component plane
    svc = sign(sMap.valclust+1); 
    tmp_h=som_grid(lattice, msize, 'surf', sMap.density.*svc+svc, ...
	'Marker', 'none', 'Line', 'none');
    set(h_axes(i),'Tag','CplaneI');
    h_label(i,1)=xlabel('Densité étendue interpolée');
