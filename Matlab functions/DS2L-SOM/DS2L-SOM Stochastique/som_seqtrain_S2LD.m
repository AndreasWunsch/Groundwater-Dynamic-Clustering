function [sMap, sTrain] = som_seqtrain_S2LD(sMap, D, varargin)
...
% Modifié par Guénaël CABANES (2008)
...
Ti = 10; Tf= 100;
ngconnect = sMap.ngconnect;
density = zeros(munits,1);

if strcmp(sTopol.lattice,'hexa'); comax = 6; else comax = 4; end;
...
%   TT = Ti*(Tf/Ti)^(sMap.t/trainlen);
  ...
  mask2 = udist(:,bmu);
  [qerr2 bmu2] = min(((Dx.^2)*mask(known)).*mask2);
  ...
  nb_bmu_con = udist(:,bmu); nb_bmu_con(nb_bmu_con==inf)=0; nb_bmu_co = sum(nb_bmu_con);
  
  R = (nb_bmu_co/comax)/(1+exp(-(20*t/trainlen-10)));
  delta = 0.01;
  
  ngconnect(bmu,bmu2) = (ngconnect(bmu,bmu2)) + R;
  ngconnect(bmu2,bmu) = (ngconnect(bmu2,bmu)) + R;
  
  for P = 1: size(nb_bmu_con,1);
        if nb_bmu_con(P,:)==1 && ~(nb_bmu_con(P,:)==bmu2);
             ngconnect(bmu,P) = max(0,ngconnect(bmu,P) - delta * R);
             ngconnect(P,bmu) = max(0,ngconnect(P,bmu) - delta * R); 
        end;
  end;
  ...
  %hde =  1/(pi*r(ind)^2) * (1-(Ud(:,bmu)/(we*r(ind)))) .* exp(-Ud(:,bmu)/(we*r(ind)));
  %hde =  exp(-Ud(:,bmu)/(we*r(ind)));
  %g =  1/(pi*r(ind)^2) * (1-(((Dx.^2)*mask(known))/(wgp*r(ind)))) .* exp(-((Dx.^2)*mask(known))/(wgp*r(ind)));
  g =  exp(-((Dx.^2)*mask(known))/(wg*r(ind)));
  
%   Dgh = M(:,known);
%   for pas=1:size(M,1); Dgi(pas,:)=M(bmu,:); end;
%   Dg = Dgh - Dgi;
%   g =  exp(-((Dg.^2)*mask(known))/(wg*r(ind)));
  
  Rde = 1/(1+exp(-(20*t/trainlen-10)));

  %density=density + g.*hde.*Rde;
  density=density + g.*Rde;
  
end; % for t = 1:trainlen

if sTrain.alpha_ini==0.05;
    
    sMap.ngconnect=ngconnect;
    
    sMap.valclust = S2L_Clust(sMap, sMap.ngconnect); 
...
%     h = exp(-Ud/(2*rr));
%     
%     d =sum(sMap.ngconnect,2)./(Um(:).^2);
%     
%     sMap.density =  sum(h * d,2);

    sMap.density = density;
