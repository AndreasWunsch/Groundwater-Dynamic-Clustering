function sMap = som_make_S2LD(D, varargin)
...
sMap.t=0;
[dim1 dim2] = size(sMap.codebook);
% sMap.ngconnect = zeros(dim1,dim1)-1;
sMap.ngconnect = zeros(dim1,dim1);
...
switch func,
 case 'seq',    sMap = som_seqtrain_S2LD(sMap,D,sTrain,'tracking',tracking,'mask',mask);
...
switch func,
 case 'seq',    sMap = som_seqtrain_S2LD(sMap,sD,sTrain,'tracking',tracking,'mask',mask);
 case 'sompak', sMap = som_sompaktrain(sMap,D,sTrain,'tracking',tracking,'mask',mask);
 case 'batch',  sMap = som_batchtrain(sMap,D,sTrain,'tracking',tracking,'mask',mask);
end
